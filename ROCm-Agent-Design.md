...
│   ├── interaction_config.yaml     #   多轮交互配置
...
全部在 4x W7800 上完成，使用 GRPO（无需 Critic，适配 128GB 总显存），TRL GRPOTrainer + accelerate：
...
使用 TRL GRPOTrainer（纯 PyTorch，不依赖 vLLM/Ray，ROCm 兼容性最好）。

奖励函数：0.0（无效）、0.1（有代码块）、0.5（有 HIP 内核代码）、1.0（有 ModelNew + HIP 内核）。
阶段 3 使用 `hip_kernel_interaction.py` 的完整编译/验证/跑分奖励。
...
accelerate launch --num_processes=4 tools/train_grpo.py \
  ...
  --output-dir checkpoints/phase1_moe_grpo \
  --batch-size 4 \
  --num-generations 2 \
  ...
  --lr 1e-6 \
  --epochs 5
# 模型 B 差异：--model models/Qwen3.5-27B --output-dir checkpoints/phase1_dense_grpo

# ========== 3. 阶段 2：RFT 过滤 + 监督微调 ==========
python3 tools/filter_trajectories.py \
  --input checkpoints/phase1_moe_grpo/ \
  --output data/rocm_agent_ops/rft_filtered.parquet \
  --min-reward 0.5

# SFT 使用 transformers Trainer（TRL SFTTrainer 或自定义脚本）
# 待实现：tools/train_sft.py

# ========== 4. 阶段 3：多轮 Agentic RL ==========
# 使用 tools/hip_kernel_interaction.py 的编译/验证/跑分奖励
# 在 TRL GRPOTrainer 外层套多轮交互循环
# 待实现：tools/train_grpo_multiturn.py
```
...
| RDNA 3 RL 训练框架适配               | 使用 TRL GRPOTrainer（纯 PyTorch）；已验证 4 卡 GRPO 训练通过                           |
...
| 推理       | transformers generate（可选 vLLM 加速）     | Apache 2.0 |
