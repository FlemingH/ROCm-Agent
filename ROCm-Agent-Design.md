...
基于强化学习训练 Agent，自动将 PyTorch 模型转化为 AMD GPU 上的高性能 HIP 内核，超越 `torch.compile` 性能。参考 CUDA-Agent 论文的 Agent 循环（实现 → 编译 → 验证 → 性能分析 → 迭代优化），按架构分版本适配 ROCm 7.x 工具链。首版聚焦 RDNA 3（gfx1100 / Radeon Pro W7800），后续扩展至 RDNA 4 等消费级/工作站架构。
...
使用 [ROCm/rocm-libraries therock-7.11](https://github.com/ROCm/rocm-libraries/releases/tag/therock-7.11) 作为 HIP 编程参考。优势：
...
| 参考代码 | rocm-libraries therock-7.11（官方源码） | MIT |
